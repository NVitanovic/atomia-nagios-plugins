...
.DESCRIPTION
    ...
    Modified by nikola.vitanovic@atomia.com for use in Atomia checks.
...
{
    #Terminate previously running procmon
...
$FileDate = Get-Date -Format "yyyy-MM-dd_HHmmss"
$TempPML = ".\Temp_$FileDate.pml"
$TempCSV = ".\FileLog_$FileDate.csv"
...
# Force creation of log folder
# Copy all newly generated CSV files to the log folder
# if the logFolder is specified to some different path
if($logFolder)
{
    New-Item -ItemType Directory -Force -Path $logFolder | Out-Null
    Copy-Item -Path *.csv -Filter *.csv -Destination $logFolder
    Remove-Item -Path *.csv -Filter *.csv
}

#Go back from whence we came and return fruits of our labor
Pop-Location

#.\procmon_logger.ps1 -ProcMonFolder . -Filter 'Path,is,D:\install.ini,Include;Operation,is,ReadFile,Include;Operation,is,WriteFile,Include'
#./procmon_logger -ProcMonFolder . -Filter 'Path,is,D:\install.ini,Include;Operation,is,ReadFile,Include;Operation,is,WriteFile,Include' -Duration 30 -logFolder C:\Users\nivit\Documents\GIT\atomia-nagios-plugins\check_file_access\logs\
